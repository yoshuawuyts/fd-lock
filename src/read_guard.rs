...
#[derive(Debug)]
pub struct FileLockReadGuard<'lock, T: sys::AsRaw> {
    guard: sys::FileLockReadGuard<'lock, T>,
}

impl<'lock, T: sys::AsRaw> FileLockReadGuard<'lock, T> {
    pub(crate) fn new(guard: sys::FileLockReadGuard<'lock, T>) -> Self {
        Self { guard }
    }
}
